...
import { StreamingDataProvider } from './streaming-data-provider';	// Import the StreamingDataProvider
...
export class UDFCompatibleDatafeedBase implements IExternalDatafeed, IDatafeedQuotesApi, IDatafeedChartApi {
	...
	private readonly _streamingURL: string; // Add streaming URL member
	...
	private readonly _streamingDataProvider: StreamingDataProvider; // Add streamingDataProvider member

	protected constructor(
		datafeedURL: string,
		streamingURL: string, // Add streaming URL parameter
		...
		this._streamingURL = streamingURL; // Assign the streaming URL
		...
		this._streamingDataProvider = new StreamingDataProvider(this._streamingURL); // Instantiate the StreamingDataProvider
	}
	...
	public subscribeBars(symbolInfo: LibrarySymbolInfo, resolution: ResolutionString, onTick: SubscribeBarsCallback, listenerGuid: string, _onResetCacheNeededCallback: () => void): void {
		// check if SymbolInfo data status is streaming or not
		if (symbolInfo.data_status !== 'streaming') 
		{
			this._dataPulseProvider.subscribeBars(symbolInfo, resolution, onTick, listenerGuid);
		}
		else 
		{
			this._streamingDataProvider.subscribeBars(symbolInfo, resolution, onTick, listenerGuid);
		}
	}

	public unsubscribeBars(listenerGuid: string): void {
		const symbolInfoPromise = this._symbolsStorage?.resolveSymbol(listenerGuid);
		symbolInfoPromise?.then((symbolInfo: LibrarySymbolInfo) => {
			if (symbolInfo.data_status !== 'streaming') {
				this._dataPulseProvider.unsubscribeBars(listenerGuid);
			} else {
				this._streamingDataProvider.unsubscribeBars(listenerGuid);
			}
		});
